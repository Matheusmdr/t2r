...
import { BarChart3, MapPin, Monitor, Play, Share2 } from 'lucide-react';
...
function StatItem({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <span className="mb-2 text-4xl font-bold text-[#333]">
        {animatedValue}
      </span>
...
function ServiceItem({
  ...
  return (
    <div className="group flex cursor-default flex-col items-center">
      <div className="mb-6 text-[#333] transition-transform duration-300 group-hover:scale-110">
        {icon}
      </div>
      <span className="max-w-35 text-center text-[11px] leading-tight font-bold tracking-wide text-[#333] uppercase">
        {title}
      </span>
    </div>
  );
    ...
    <MainLayout>
      <main className="min-h-screen w-full bg-[#f2f2f2] font-sans">
        ...
        <section className="mx-auto flex max-w-5xl flex-col items-center px-6 py-20 text-center">
          ...
          <div className="group relative aspect-video w-full max-w-4xl overflow-hidden rounded-sm bg-black shadow-2xl">
            <img
              src="https://placehold.co/1280x720/333/white?text=Apresentação+T2R"
              className="h-full w-full object-cover opacity-90 transition-opacity group-hover:opacity-70"
              alt="Thumbnail"
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex h-16 w-16 cursor-pointer items-center justify-center rounded-full border border-white/20 bg-black/50 text-white backdrop-blur-sm transition-transform hover:scale-110">
                <Play className="ml-1 fill-current" size={24} />
              </div>
            </div>
            <div className="absolute right-0 bottom-0 left-0 h-1.5 bg-gray-800">
              <div className="h-full w-[35%] bg-white/40" />
            </div>
          </div>
        </section>
      </main>
